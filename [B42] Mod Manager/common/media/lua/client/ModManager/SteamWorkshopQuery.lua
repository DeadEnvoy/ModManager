...
function SteamWorkshopQuery.OnSteamQueryCompleted(status, info)
    ModManagerCache.isQuerying = false; if status == "Completed" then
        ModManagerCache:updateWorkshopData(info, SteamWorkshopQuery.modMap)
        ModManagerCache:updateUsageStats(SteamWorkshopQuery.queriedCount or 0)
        ModManagerCache:save(); triggerEvent("OnModManagerWorkshopUpdate")
    end
end
...
function SteamWorkshopQuery.query()
    ...
    if not workshopIDs:isEmpty() then
        local cache = ModManagerCache:load()
        local now = os.time()
